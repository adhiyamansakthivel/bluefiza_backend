...
class ManageNavCategories extends Component
{
    public $navkey;
    public $categoryname ='';
    public $catslugValue = '';
    ...
    {
        $navigation = BlNavigation::where('bln_key',$this->navkey)->firstOrFail(); 
    ...
    {
        $openModel =  BlCategory::join('bl_navigations', 'bl_navigations.id', 'blcnav_id')
        ->where('blc_key', $id)->where('bln_key',$this->navkey)->firstOrFail();
    ...
    {
        $post = BlCategory::join('bl_navigations', 'bl_navigations.id', 'blcnav_id')
        ->where('blc_key', $id)->firstOrFail();

        $navig = BlNavigation::where('bln_key', $this->navkey )->firstOrFail();
    ...
    public function render()
    {
        ...
        ->where('blcnav_id', $navigation->id)
        ->get();

        return view('livewire.admin.navigation.manage-nav-categories', compact('categoryList', 'navigation'));
    }
}
